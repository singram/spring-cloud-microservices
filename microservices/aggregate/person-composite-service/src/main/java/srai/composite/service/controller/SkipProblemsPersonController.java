...
@RestController
@RequestMapping("/skip_problems")
public class SkipProblemsPersonController {

  private static final Logger LOG = LoggerFactory.getLogger(SkipProblemsPersonController.class);
  ...
  public ResponseEntity<PersonComposite> getProduct(@PathVariable int personId) {
    ...
    try {
      ResponseEntity<Recommendation[]> recommendationResult = personRecommendationsServiceGateway.getPersonRecommendationsSP(personId);
    ...
    try {
      ResponseEntity<Recommendation[]> recommendationResult = productRecommendationsServiceGateway.getProductRecommendationsSP(personId);
